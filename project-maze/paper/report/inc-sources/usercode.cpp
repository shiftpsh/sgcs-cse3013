...
bool readFile(const char *filename) {
    ...
    for (int i = 0; i < rows; i++)
        checked_cells[i].resize(cols);
    for (int i = 0; i < rows; i++)
        visited_cells[i].resize(cols);
    for (int i = 0; i < rows; i++)
        shortest_route[i].resize(cols);

    // maze has floor(rows / 2) calls and floor(rows / 2) + 1 walls vertically;
    // floor(cols / 2) calls and floor(cols / 2) + 1 walls horizontally.
    setWindow(0, 0, CELL_SIZE * (cols / 2) + WALL_WIDTH * (cols - cols / 2),
              CELL_SIZE * (rows / 2) + WALL_WIDTH * (rows - rows / 2), 1);
...
static void drawBuffered() {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            ...
            COLORREF color = RGB(0x00 * (1 - blend) + 0x9c * blend,
                                 0xbc * (1 - blend) + 0x27 * blend,
                                 0xd4 * (1 - blend) + 0xb0 * blend);

            switch (field[i][j]) {
            case '+':
                DrawSolidBox_I(y, x, y + WALL_WIDTH, x + WALL_WIDTH, 0, color,
                               color);
                break;
            case '-':
                DrawSolidBox_I(y, x, y + CELL_SIZE, x + WALL_WIDTH, 0, color,
                               color);
                break;
            case '|':
                DrawSolidBox_I(y, x, y + WALL_WIDTH, x + CELL_SIZE, 0, color,
                               color);
                break;
            }
        }
    ...
    // Draw checked routes
    for (int i = 0; i < rows; i++) {
        double x = CELL_SIZE * (i / 2) + WALL_WIDTH * (i - i / 2) +
                   (double)CELL_SIZE / 2 - (double)WALL_WIDTH / 2;
        for (int j = 0; j < cols; j++) {
            double y = CELL_SIZE * (j / 2) + WALL_WIDTH * (j - j / 2) +
                       (double)CELL_SIZE / 2 - (double)WALL_WIDTH / 2;

            // this adds gradients just for fun
            double blend = (double)(i + j) / (rows + cols);
            COLORREF color = RGB(0xdd * (1 - blend) + 0xee * blend,
                                 0xdd * (1 - blend) + 0xee * blend,
                                 0xdd * (1 - blend) + 0xee * blend);

            if (checked_cells[i][j]) {
                if ((i & 1) && (j & 1)) {
                    DrawSolidBox_I(y, x, y + WALL_WIDTH, x + WALL_WIDTH, 0,
                                   color, color);
                } else if (i & 1) {
                    DrawSolidBox_I(y - CELL_SIZE + WALL_WIDTH, x,
                                   y + WALL_WIDTH, x + WALL_WIDTH, 0, color,
                                   color);
                } else {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            ...
            // this adds gradients just for fun
            double blend = (double)(i + j) / (rows + cols);
            COLORREF color = RGB(0xff - (0x00 * (1 - blend) + 0x9c * blend),
                                 0xff - (0xbc * (1 - blend) + 0x27 * blend),
                                 0xff - (0xd4 * (1 - blend) + 0xb0 * blend));
...
void DFS() {
    for (int i = 0; i < rows; i++)
        std::fill(checked_cells[i].begin(), checked_cells[i].end(), false);
    for (int i = 0; i < rows; i++)
        std::fill(visited_cells[i].begin(), visited_cells[i].end(), false);
    for (int i = 0; i < rows; i++)
        std::fill(shortest_route[i].begin(), shortest_route[i].end(), false);
