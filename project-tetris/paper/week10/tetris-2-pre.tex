...
이진 탐색 트리는 삽입과 삭제에 걸리는 시간은 일반적으로 $\mathcal{O}\left(\log n\right)$이며,
중위 순회($\mathcal{O}\left(n\right)$)할 경우 항상 정렬된 상태로 순회하게 되는 특징이 있다.
...
\proc{BST-Get}: $n$번째 순위의 노드를 가져온다.
